...
client.on('message', (message) => {
	...
	if (config.command.notfound_message === true && !client.commands.has(command)) {
        return message.channel.send(
            ...
            .setDescription(`Lo Siento ese comando no existe \`${command}\` intenta otro`)
	...
	try {
		...
		if (config.command.error_message === true) {
            message.channel.send(
                new Discord.MessageEmbed()
                .setColor(config.color.red)
                .setTitle('Un Error Paso')
                .setDescription(`Un Error ocurrio cuando ejecutaste \`${command}\` intenta denuevo`)
                .addField('Error', `\`\`\`js\n${error}\n\`\`\``)
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                .setTimestamp()
            );
