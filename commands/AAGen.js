...
module.exports = {
	...
	description: 'Generar una Cuenta de algun Servicio', // Command description
        ...
        if (message.channel.id === config.genChannel) {
            ...
            if (generated.has(message.author.id)) {
                return message.channel.send(
                    ...
                    .setColor(config.color.red)
                    .setTitle('Cooldown!')
                    .setDescription('Espera 10 segundos desde la ultima ves que generaste algo')
                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                    .setTimestamp()
                );
            } else {
                ...
                if (!service) {
                    return message.channel.send(
                        ...
                        .setDescription('Especifica el nombre del servicio')
                        .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                        .setTimestamp()
                    );
                };
                    ...
                    if (!error) {
                        ...
                        if (position === -1) {
                            return message.channel.send(
                                new MessageEmbed()
                                .setColor(config.color.red)
                                .setTitle('Error del Gen')
                                .setDescription(`No me queda mas Stock de \`${args[0]}\` avisa al staff o espera y habra mas Stock!`)
                                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                .setTimestamp()
                            );
                        };
                        ...
                        .then(message.author.send('Aca esta Copia y Pega:'))
                        ...
                        if (position !== -1) {
                            ...
                            fs.writeFile(filePath, data, function (error) {
                                message.channel.send(
                                    new MessageEmbed()
                                    .setColor(config.color.green)
                                    .setTitle('Cuenta Generada Correctamente')
                                    .setDescription(`Mira Md ${message.author}! *Si no recibiste nada al Md ponelo en publico*`)
                                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                    .setTimestamp()
                                );
                            ...
                            return message.channel.send(
                                ...
                                .setColor(config.color.red)
                                .setTitle('Error del Gen')
                                .setDescription(`El \`${args[0]}\` servicio esta agotado`)
                                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                .setTimestamp()
                            );
                        };
                        ...
                        return message.channel.send(
                            ...
                            .setColor(config.color.red)
                            .setTitle('Error del Gen')
                            .setDescription(`El Servicio \`${args[0]}\` no existe prueba otro`)
                            .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                            .setTimestamp()
                        );
                    };
            ...
            message.channel.send(
                new MessageEmbed()
                .setColor(config.color.red)
                .setTitle('No Puedes usar el Gen aqui')
                .setDescription(`No podes usar el \`Gen\` usa el comando en <#${config.genChannel}>!`)
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                .setTimestamp()
            );
        };
